...
def update_user(user_id: int, user: UserToUpdate = fastapi.Body(), current_user: Admin = Depends(get_current_user)):
...
def create_user(user: UserToCreate, current_user: Admin = Depends(get_current_user)):
    """
    Создаем Юзера
    :param user:
    :return:
...
@crud.db_session
def create_transaction(user_id, transaction: CreateTransaction, current_user: Admin = Depends(get_current_user)):
